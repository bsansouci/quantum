...
quantum_measure(quantum_reg reg)
{
  ...
  double r;
  int i;
  // printf("Measure is called! With size %d\n", reg.size);
  // printf("%d, %d, %d, %llu, %p\n", reg.width, reg.size, reg.hashw, reg.node[0].state, reg.hash);
  if(quantum_objcode_put(MEASURE)){
    // printf("Returning 0 in quantum_measure!\n");
    return 0;
  }
  ...
  for (i=0; i<reg.size; i++)
    {
	 ...
	 result. Otherwise, continue with the next base state. */
      ...
      if(0 >= r){
        // printf("Returning state %llu at i %d in quantum_measure!\n", reg.node[i].state, i);
